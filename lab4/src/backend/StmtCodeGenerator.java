...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		...
		Unit stmt = j.newGotoStmt(breakTargets.get(wStmt));
		units.add(stmt);
		return null;
	}
	...
	public Void visitWhileStmt(WhileStmt nd) {
		...
		// emit if condition == 0, go to l1
		units.add(j.newIfStmt(j.newEqExpr(condition, IntConstant.v(0)), l1));
